...
handle_other({'EXIT', _Parent, normal}, State = #v1{queue_collector = Collector}) when ?IS_RUNNING(State) ->
    %% rabbitmq/rabbitmq-server#544
    %% The connection port process has exited due to the TCP socket being
    %% closed. We can't send a closed message to the existing channels, so
    %% clean them up and exit
